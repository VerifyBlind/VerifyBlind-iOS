...
final class CameraController: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    /// Aynı karenin `CMSampleBuffer`'ı — ML Kit için AYRI kanal.
    ///
    /// `VisionImage`'ın `CVPixelBuffer` init'i YOK, yalnız `CMSampleBuffer` kabul ediyor. `onFrame`'in
    /// imzasını genişletmek MRZ/QR tarayıcıları dahil beş çağrı yerini birden kırardı; liveness dışı
    /// ekranlara dokunmamak için ikinci bir kanal açıldı. İkisi de AYNI kareden, AYNI video
    ...
    func runOnVideoQueue(_ block: @escaping () -> Void) {
        // Bekleme ÖLÇÜLÜYOR. Cihazda buraya verilen bir iş 40 saniye çalışmadı, üstelik aynı sürede
        // kareler kesintisiz işleniyordu (Sentry 2026-08-25). Seri bir kuyrukta bu olmamalı ve
        // sebebini açıklayamadım; liveness akışı artık bu sıçramaya bağlı değil, ama sıçrama hâlâ
        // gecikiyorsa bunu SESSİZCE yaşamak istemiyorum.
        let enqueuedAt = Date().timeIntervalSince1970 * 1000
        videoQueue.async {
            let waited = Date().timeIntervalSince1970 * 1000 - enqueuedAt
            if waited > 2000 {
                Log.warning("Video kuyruğu \(Int(waited)) ms bekletti (koşan kuyruk: \(Self.currentQueueLabel))",
                            category: .liveness)
            }
            block()
        }
    ...
    /// Şu an hangi kuyruktayız — kare işleme ile `runOnVideoQueue` gerçekten AYNI kuyrukta mı?
    ...
    private func configure() {
        ...
        if let conn = output.connection(with: .video) {
            // `videoOrientation` iOS 17'de DEPRECATED oldu ve bazı yapılandırmalarda
            // `isVideoOrientationSupported` false dönüyor — o zaman döndürme SESSİZCE uygulanmaz
            // ve buffer sensör yöneliminde (yatay) kalır. Vision bunu tolere ediyordu (döndürülmüş
            // yüzleri de bulur, FaceAligner da gözlerden hizalar); ML Kit etmiyor, 90° sapmada hiç
            // yüz döndürmez. Bu yüzden modern API bir YEDEK olarak eklendi.
            //
            // Sıra bilinçli: çalışan yol (`videoOrientation`) ÖNCE denenir → mevcut davranış
            // birebir korunur, yeni yol yalnız eskisi reddedildiğinde devreye girer.
            if conn.isVideoOrientationSupported {
                conn.videoOrientation = .portrait
            } else if #available(iOS 17.0, *), conn.isVideoRotationAngleSupported(90) {
                conn.videoRotationAngle = 90   // portre
                Log.warning("CameraController: videoOrientation desteklenmiyor — videoRotationAngle=90 kullanıldı", category: .liveness)
            } else {
                Log.warning("CameraController: video döndürme UYGULANAMADI — buffer sensör yöneliminde kalıyor", category: .liveness)
            }
    ...
    func captureOutput(_ output: AVCaptureOutput,
        ...
        // .portrait + (ön kamerada) ayna uygulandığı için buffer dik → Vision/ML Kit .up.
        onFrame?(pixelBuffer, .up)
        onSampleBuffer?(sampleBuffer, .up)
    }
}
