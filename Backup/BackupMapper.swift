...
enum BackupManager {
    ...
    /// Yazma hatası YUTULMAZ (Android `BackupManager.importRecords` paritesi) — aksi hâlde hiçbir
    /// kayıt yazılmadan "N eklendi" gösterilirdi. Çağıran hatayı kullanıcıya bildirir.
    static func importRecords(_ repo: HistoryRepository = .shared, _ incoming: [BackupRecord]) throws -> ImportResult {
        let localNonces = repo.getAllNonces()
        let toAdd = BackupMapper.selectNewRecords(incoming, localNonces: localNonces)
        for r in toAdd { try repo.insertBackupRecord(r) }
