...
/// Android `backup/CloudFileEntry`.
struct CloudFileEntry {
    let name: String
    /// Sağlayıcının bildirdiği değişiklik zamanı (ms); bilinmiyorsa nil (liste sıralaması).
...
protocol CloudProvider {
    ...
    /// Manuel geri-yükleme akışının dosya seçtirme adımı için (Android `list`).
