...
enum FaceAligner {
    ...
    /// Gözleri X'e göre sıralar: hedef sabitler (`dstLeftEye.x` < `dstRightEye.x`) görüntünün
    /// SOLUNDAKİ gözü bekliyor, "öznenin solu"nu değil.
    ///
    /// Bu ayrım kritik: `params` açıyı `atan2(dy, dx)` ile hesaplıyor, yani sıra ters gelirse
    /// `dx` işaret değiştirir ve yüz 180° DÖNÜK hizalanır — embedding çöp olur, eşleşme sıfıra iner.
    /// Vision `landmarks.leftEye` ile MLKit `FaceLandmark.leftEye`'ın konvansiyonu aynı olmak
    /// zorunda değil ve ön kamera aynası hangi gözün görüntünün solunda kaldığını da çevirir.
    /// Sıralamak bu belirsizliğin tamamını kaldırır: doğru sırada gelen girdi için sonuç
    /// DEĞİŞMEZ, ters gelen girdi ise sessizce bozulmak yerine düzelir.
    static func orderedByX(_ a: CGPoint?, _ b: CGPoint?) -> (left: CGPoint?, right: CGPoint?) {
        guard let a, let b else { return (a, b) }
        return a.x <= b.x ? (a, b) : (b, a)
    }

    static func params(leftEye: CGPoint?, rightEye: CGPoint?) -> AlignmentParams {
        let (leftEye, rightEye) = orderedByX(leftEye, rightEye)
    ...
    static func alignedImage(from source: CGImage, leftEye: CGPoint?, rightEye: CGPoint?) -> CGImage? {
        // params() ile AYNI sıralama — aşağıdaki transform doğrudan `leftEye!`'ı kullanıyor,
        // sırayı burada da düzeltmezsek açı ile öteleme farklı gözlere dayanır.
        let (leftEye, rightEye) = orderedByX(leftEye, rightEye)
