...
struct LoginFlowView: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .failed(let title, let message, let offersReset):
                FailedStepView(
                    title: title,
                    message: message,
                    onReset: resetAction(enabled: offersReset),
                    onClose: {
                        vm.openReturnIfValid(status: "cancelled")
                        onFinish()
                    })
    ...
    /// Okunamayan kart verisini temizleyen "Sil" eylemi — yalnız [enabled] iken sunulur.
    ///
    /// Temizlik sonrası cüzdanı elle tazelemek gerekmiyor: akış kapanınca RootView'in
    /// `fullScreenCover(onDismiss:)`'i zaten `appState.refresh()` çağırıyor.
    private func resetAction(enabled: Bool) -> (() -> Void)? {
        guard enabled else { return nil }
        return {
            vm.resetCardData()
            onToast(L.t("card_data_cleared"))
            vm.openReturnIfValid(status: "cancelled")
            onFinish()
        }
    }
