...
struct HistoryView: View {
    ...
    /// Silme onayı bekleyen kayıt — Android `HistoryFragment` sola-kaydırma diyaloğu paritesi.
    ...
    private var mainContent: some View {
        ...
        .onAppear {
            vm.load()
            showBackupBanner = true   // manuel Yedekle/Geri Yükle modelinde banner daima görünür giriş noktası
        }
    ...
    /// Banner görünürlük kuralı — manuel Yedekle/Geri Yükle modelinde banner DAİMA görünür bir
    /// giriş noktasıdır (sürekli bağlantı durumu yok). Regresyon testi `SmokeTests` buradan sürer.
    static func shouldShowBackupBanner() -> Bool { true }
    ...
    private var list: some View {
        List {
            ForEach(vm.records, id: \.id) { rec in
                HistoryRowView(record: rec, showDevice: vm.showDevice)
        ...
        // Silme onayı — Android `delete_record_title`/`delete_record_message` diyaloğunun aynısı.
        // Silme SERT: yerel satır kalıcı gider (yedek dosyasından geri yüklenebilir).
        // Revoke diyaloğuyla AYNI view'a iliştirilmez — iki presentation modifier'ı aynı view'da
        // çakışıyor (aynı tuzak `BackupSettingsView`'de de not edilmiş).
        .confirmationDialog(L.t("delete_record_title"), isPresented: Binding(
            get: { pendingDelete != nil }, set: { if !$0 { pendingDelete = nil } }
        ), titleVisibility: .visible) {
            if let rec = pendingDelete {
                Button(L.t("btn_delete_confirm"), role: .destructive) {
                    vm.delete(rec)
                    pendingDelete = nil
                }
                Button(L.t("btn_cancel"), role: .cancel) { pendingDelete = nil }
            }
        } message: {
            Text(L.t("delete_record_message"))
        }
    }
...
private struct HistoryRowView: View {
    ...
    /// Yalnız listede birden fazla cihaz varsa cihaz adı satırı gösterilir.
        ...
        HStack(alignment: .top, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(resolvedDesc)
                    .font(.system(size: 14))
                    .foregroundColor(Theme.onSurfaceVariant)
                if showDevice, !record.deviceName.trimmingCharacters(in: .whitespaces).isEmpty {
