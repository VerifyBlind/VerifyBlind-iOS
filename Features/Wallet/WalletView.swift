...
struct WalletView: View {
    ...
    @State private var showBackupBeforeDelete = false
    ...
    var body: some View {
        ...
        .confirmationDialog(L.t("delete_confirm_title"), isPresented: $showDeleteConfirm, titleVisibility: .visible) {
            ...
            Button(L.t("delete_confirm_also_history"), role: .destructive) {
                showBackupBeforeDelete = true
            }
            Button(L.t("btn_cancel_upper"), role: .cancel) {}
        } message: {
            Text(L.t("delete_confirm_desc"))
        }
        // "İşlem geçmişimi de sil" seçilince: silmeden ÖNCE yedek teklifi (Android
        // `WalletFragment.offerBackupBeforeDelete` paritesi). "Yedekle" → yedekleme ekranına gider,
        // silme İPTAL olur (yedekten sonra tekrar silebilir); "Yine de sil" → doğrudan siler.
        .confirmationDialog(L.t("delete_backup_prompt_title"), isPresented: $showBackupBeforeDelete, titleVisibility: .visible) {
            Button(L.t("delete_backup_prompt_backup")) { onBackup?() }
            Button(L.t("delete_backup_prompt_delete"), role: .destructive) {
                Task { await removeIdentity(deleteHistory: true) }
            }
            Button(L.t("btn_cancel"), role: .cancel) {}
        } message: {
            Text(L.t("delete_backup_prompt_message"))
        }
    ...
    /// Kartı kaldırır. [deleteHistory] true ise o karta ait TÜM işlem geçmişi de SERT silinir
    /// (tombstone yok — manuel Yedekle/Geri Yükle modelinde silme yereldedir; kullanıcı silmeden
    /// önce yedeklemeye yönlendirilir, bkz. [offerBackupBeforeDelete]).
    ///
    /// Neden seçenek: geçmiş normalde kart silinince DURUR — liste cardId ile filtrelendiği için
    /// görünmez olur ve aynı kart tekrar eklenince geri gelir. Silmek isteyen için "gizlendi ≠
    /// silindi" yanılgısı yaratıyordu → artık kullanıcı açıkça seçiyor.
    private func removeIdentity(deleteHistory: Bool) async {
        ...
        if deleteHistory, let cid = cardId, !cid.isEmpty {
            HistoryRepository.shared.deleteByCardId(cid)
        } else {
        ...
        // Silme sessizce olup bitmemeli: kullanıcı yıkıcı bir işlemin gerçekten yapıldığını
        // görmeli (Android `card_deleted_toast` / `card_and_history_deleted_toast` paritesi).
        appState.showToast(L.t(deleteHistory ? "card_and_history_deleted_toast" : "card_deleted_toast"))
        appState.refresh()
    }
}
