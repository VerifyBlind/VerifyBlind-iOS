        ...
        ZStack {
            ...
            if camera.permissionDenied || camera.configurationFailed {
                ...
                GeometryReader { geo in
                    ...
                    // Bottom instructions
                    VStack(spacing: 4) {
                        // "ARKA" amber+bold: kırmızı kamera ekranında hata sinyali okunur.
                        (Text(L.t("scan_mrz_instruction_prefix"))
                            .font(.system(size: 16, weight: .semibold)).foregroundColor(.white)
                         + Text(L.t("scan_mrz_instruction_emphasis"))
                            .font(.system(size: 19, weight: .heavy))
                            .foregroundColor(Color(red: 1, green: 0.702, blue: 0.192))  // #FFB331
                         + Text(L.t("scan_mrz_instruction_suffix"))
                            .font(.system(size: 16, weight: .semibold)).foregroundColor(.white))
                            .accessibilityLabel(L.t("scan_mrz_instruction"))
...
private struct MRZCardHint: View {
    ...
    /// Bu genişliğin altında kart okunaksız hale gelir; hiç göstermemek daha iyi.
    private static let minCardW: CGFloat = 120
