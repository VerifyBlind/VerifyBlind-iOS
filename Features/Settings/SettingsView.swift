...
struct SettingsView: View {
            ...
            ScrollView {
                VStack(spacing: 8) {
                    ...
                    navRow(icon: "text.bubble", fill: Theme.cyanSoft, tint: Theme.chipCyan,
                           title: "settings_faq_title", desc: "settings_faq_desc", action: onFaq)

                    navRow(icon: "envelope", fill: Theme.cyanSoft, tint: Theme.chipCyan,
                           title: "settings_feedback_title", desc: "settings_feedback_desc") {
                        showFeedback = true
                    }
    ...
    /// Yedekleme satırı — sürekli bağlantı durumu YOK (manuel Yedekle/Geri Yükle). Statik başlık;
    /// eylemler açılan `BackupView`'da.
    ...
    /// Kabul edilen hukuki metinler — sürüm + tarih. Zero-knowledge mimaride bu kayıt yalnızca
    /// cihazda durduğu için kullanıcının neyi ne zaman kabul ettiğini görebilmesi gerekir
    /// (Android SettingsFragment `cardLegalTerms` paritesi).
    private var legalTermsRow: some View {
        Button(action: openTerms) {
            CardSurface {
                HStack(spacing: 16) {
                    IconCircle(systemName: "doc.text", fill: Theme.cyanSoft, tint: Theme.chipCyan)
                    VStack(alignment: .leading, spacing: 2) {
                        Text(L.t("legal_terms_settings_row"))
                            .font(.system(size: 14, weight: .bold)).foregroundColor(Theme.onSurface)
                        Text(LegalTerms.acceptanceSummary ?? L.t("legal_terms_settings_none"))
    ...
    private func openTerms() {
        ...
        if let url = URL(string: "https://verifyblind.com/\(lang)/terms") {
