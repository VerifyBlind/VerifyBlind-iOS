...
final class PassportNFCReader {
    ...
    func read(documentNumber: String, dateOfBirth: String, dateOfExpiry: String,
              documentType: String = "ID", handshakeNonce: String) async throws -> ScannedPassport {
        guard NFCReadError.readingAvailable else { throw NFCReadError.notAvailable }

        // NFC sistem sheet talimatı belge tipine göre (Android `detectedDocumentType` paritesi).
        let presentInstruction = (documentType == "PASSPORT")
            ? L.t("nfc_passport_instruction") : L.t("nfc_id_card_instruction")
        ...
        do {
            ...
            model = try await reader.readPassport(
                ...
                customDisplayMessage: { msg in
                    switch msg {
                    case .requestPresentPassport: return presentInstruction
