...
enum Theme {
    ...
    // Wallet kartı (bg_wallet_card_redesign): diyagonal 3-duraklı derin lacivert (sol-üst ışık).
    static let walletCardGradient = LinearGradient(
        colors: [Color(hex: "#15529C"), Color(hex: "#0C3F82"), Color(hex: "#071E44")],
        startPoint: .topLeading, endPoint: .bottomTrailing
    )
    // Kartın üstünde yumuşak parlama (sheen) — üst kenarda ışık, ~%45'te söner.
    static let walletCardSheen = LinearGradient(
        stops: [.init(color: Color(hex: "#22FFFFFF"), location: 0),
                .init(color: .clear, location: 0.45)],
        startPoint: .top, endPoint: .bottom
    )
    static let walletCardStroke  = Color(hex: "#3A78C8")
    static let walletCardShadow  = Color(hex: "#0A2A55")  // lacivert tonlu gölge (elevation 10dp)
    ...
    // Süresi dolmuş rozeti (kırmızı) — bg_badge_expired.
    static let badgeRedText   = Color(hex: "#EF4444")
    static let badgeRedFill   = Color(hex: "#1AEF4444")
    static let badgeRedStroke = Color(hex: "#33EF4444")

    // Cüzdan gövdesi radyal ışık havuzu (bg_wallet_body) — kart arkası aydınlık → kenarlar temiz.
    static let walletBodyRadial = RadialGradient(
        colors: [Color(hex: "#EAF0FB"), Color(hex: "#F5F8FC")],
        center: UnitPoint(x: 0.5, y: 0.28),
        startRadius: 0, endRadius: 620
    )

    // BETA rozeti (amber pill) — landing header ile aynı (%15 dolgu / %30 kenar).
    static let accentOrange = Color(hex: "#F59E0B")
    static let betaFill     = Color(hex: "#26F59E0B")
    static let betaStroke   = Color(hex: "#4DF59E0B")
