...
///
/// Hassas tanımlayıcıları saklar: `personId` / `cardId` (enclave'de KMS'teki gizli HMAC anahtarıyla
/// türetilir — SHA256(HMAC-SHA256(K_kms, ...)); TCKN/SOD'un doğrudan hash'i DEĞİL, dolayısıyla
/// TCKN uzayından brute-force edilemez) ve `fcm_token`. Keychain `kSecClassGenericPassword`,
/// `AfterFirstUnlockThisDeviceOnly` erişim.
enum SecureStore {
