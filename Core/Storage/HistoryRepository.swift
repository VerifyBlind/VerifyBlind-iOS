...
final class HistoryRepository {
    ...
    func insertCloudItem(_ item: HistoryRecord) {
        do {
            ...
            // deviceName buluttan çözülmüş (orijin cihaz adı) gelir → yerele yeniden şifrele.
            // Boşsa (eski yedek) boş bırak; mevcut cihaz adıyla EZME (orijin korunur).
            let encDevice = item.deviceName.isEmpty ? "" : try encryptString(item.deviceName)
            var rec = item
            rec.id = nil
            rec.title = encTitle
            rec.description = encDesc
            rec.deviceName = encDevice
            rec.isSent = true
            try db.write { db in try rec.insert(db) }
        } catch {
