...
enum BiometricGate {
    ...
    static func isDeviceLockAvailable() -> Bool {
        ...
        if !ok {
            // info: Sentry'ye EVENT göndermez, yalnız breadcrumb bırakır. Kilit tanımlı olmaması
            // beklenen bir ÇEVRE durumu (kullanıcı Ayarlar'dan düzeltir), kod arızası değil — ve
            // kilitsiz mağaza inceleme cihazları her gönderimde tetikliyor (VERIFYBLIND-IOS-3W).
            // Android tarafı aynı durumu zaten info'ya indirmişti (MainViewModel.initUserKey).
            Log.info("Cihaz kilidi kullanılamıyor: \(error?.localizedDescription ?? "bilinmiyor")", category: .flow)
        }
        return ok
    }
