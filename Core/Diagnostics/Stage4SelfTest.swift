...
/// sonuç ekrana + Sentry'e yazılır — CI'da KOŞMAZ.
...
enum Stage4SelfTest {
    ...
    static func runAll() -> [SelfTestResult] {
        ...
        // ── GRDB history: insert → fetch (decrypt) → revoke → SERT sil ──
        r.append(check("GRDB history insert/fetch/revoke/delete + şifreleme") {
            ...
            repo.deleteById(id)
            let afterDelete = repo.findByNonce(nonce) != nil   // sert silme: satır tabloda KALMAMALI
            return (decOk && revokeOk && !afterDelete,
                    "dec=\(decOk) revoke=\(revokeOk) hardDeleted=\(!afterDelete)")
        })
