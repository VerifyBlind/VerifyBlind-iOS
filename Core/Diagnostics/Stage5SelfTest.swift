    ...
    static func runAll() -> [SelfTestResult] {
        ...
        r.append(check("HistoryRepository.insertCloudItem (re-encrypt + isSent)") {
            ...
            let rec = HistoryRecord(id: nil, title: "PlainTitle", description: "PlainDesc",
                                    ...
                                    partnerId: "Acme", deviceName: "iPhone 14 Pro",
            ...
            // deviceName: yerelde şifreli (≠ düz metin), okurken orijin adı çözülür.
            let deviceRoundTrip = raw?.deviceName != "iPhone 14 Pro" && decrypted?.deviceName == "iPhone 14 Pro"
            let ok = raw?.isSent == true && raw?.title != "PlainTitle" && decrypted?.title == "PlainTitle"
                && deviceRoundTrip
            return (ok, "isSent=\(raw?.isSent ?? false) decTitle=\(decrypted?.title ?? "nil") dev=\(decrypted?.deviceName ?? "nil")")
        })
