...
/// Attestation doğrulama hatasının türü — kullanıcı mesajı seçimi için.
/// authorization = PCR0 imzası yok/eşleşmiyor (iyi huylu: sürüm/deploy boşluğu).
/// integrity     = CA zinciri / COSE / debug-mode / belge-anahtar (olası kurcalama/MITM).
enum AttestFailureKind { case authorization, integrity }

extension AttestFailureKind {
    /// Kullanıcıya gösterilecek dostça, türe-göre mesaj (ham failReason ASLA gösterilmez).
    var userMessage: String {
        switch self {
        case .authorization: return L.t("attestation_error_authorization")
        case .integrity:     return L.t("attestation_error_integrity")
        }
    }
}
...
enum AttestationVerifier {
    ...
    struct VerificationResult {
        ...
        let failureKind: AttestFailureKind?

        static func fail(_ reason: String, kind: AttestFailureKind = .integrity) -> VerificationResult {
            VerificationResult(isValid: false, failReason: reason, pcr0: nil, enclavePubKey: nil, isMockDocument: false, failureKind: kind)
        }
    }
...
"""
    ...
    private static func verifyPayload(payloadMap: [CBOR: CBOR], pcr0Signature: String?) -> VerificationResult {
        ...
        guard let sig = pcr0Signature, !sig.isEmpty else {
            return .fail("Yetkisiz Enclave: PCR0 imzası sağlanmadı", kind: .authorization)
        }
        let (authOK, authErr) = verifyPcr0Signature(pcr0: pcr0, signatureBase64: sig)
        guard authOK else {
            return .fail("PCR0 imza doğrulaması BAŞARISIZ: \(authErr)", kind: .authorization)
        }
        ...
        return VerificationResult(isValid: true, failReason: nil, pcr0: pcr0, enclavePubKey: enclavePubKey, isMockDocument: false, failureKind: nil)
    }
