...
actor AppAttestService {
    ...
    ///
    /// ⚠️ DOĞRUDAN ÇAĞIRMA — `AttestedRequest.perform` kullan. Burada üretilen imza, App Attest
    /// sayacını artırır ve sunucu kesin artan sıra bekler; imzayı üretip isteği kilit DIŞINDA
    /// göndermek iki çağrının ağda sırasını bozar ve geride kalan "sayaç gerilemesi (replay)"
    /// ile reddedilir (cihazda 2026-08-26). Kilit, üretimi VE gönderimi birlikte kapsamalı.
    ///
    ...
    func attestationHeaders(boundTo cardId: String? = nil) async -> [String: String] {
        ...
        do {
            ...
            Log.warning("App Attest token üretilemedi — graceful devam", error: error, category: .crypto)
        }
        return headers
    }
