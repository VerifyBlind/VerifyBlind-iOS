...
enum APIClientError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .http(let status, let body):
            ...
            return L.t("error_server_status", status)
        ...
        case .decoding:
            return L.t("error_response_unreadable")
        }
    }

    /// Ekran başlığı — mesajın SEBEBİYLE tutarlı olmalı.
    ///
    /// 5xx aldığında akışın kendi başlığını ("Doğrulama Başarısız") göstermek kullanıcıya kendi
    /// yaptığı bir şeyin bozulduğunu düşündürüyor; sorun bizim tarafımızdayken bunu söylemek
    /// gerekiyor. Android `ServerErrorMessages` + `error_server_unavailable_title` bunu zaten
    /// yapıyordu. nil = çağıranın akış başlığı doğru olan.
    var suggestedTitleKey: String? {
        switch self {
        case .http(let status, _) where (500...599).contains(status):
            return "error_server_unavailable_title"
        case .network:
