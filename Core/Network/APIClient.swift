...
///   önerilmez" diyor — özellikle pin setini uzaktan güncelleyemiyorsan. Sunucu tarafında bir
///   drift denetleyicisi edge zincirini izler ve CA değişiminde alarm üretir; istemciyi bloklamaz.
...
final class APIClient {
        ...
        while true {
            do {
                ...
                if Self.isRetriable(urlError), attempt < maxRetries {
                    Log.warning("APIClient: retry (\(attempt + 1)/\(maxRetries)) — \(urlError.code)", error: urlError, category: .network)
                    try await Task.sleep(nanoseconds: backoff(attempt))
                    attempt += 1
                    continue
                }
                // Geçici bağlanırlık (no internet/timeout) — retry'lar tükendi. Kod arızası değil → warning.
                Log.warning("APIClient: ağ hatası (\(endpoint.path)) — \(urlError.code)", error: urlError, category: .network)
    ...
    private func buildRequest(_ endpoint: Endpoint) -> URLRequest {
        let url = Self.url(origin: origin, path: endpoint.path)
    ...
    /// origin + path birleşimi. `buildRequest` ile testler AYNI kuralı paylaşsın diye statik.
    ///
    /// Birleştirme bilerek STRING düzeyinde yapılır: `appendingPathComponent` "?"yi encode edip
    /// query string'i bozardı. origin path taşımaz, araya sabit bir `/` konur.
    ///
    /// Baştaki slash'lar TEMİZLENİR. Path da `/` ile başlarsa URL `https://host//api/...` olur ve
    /// ASP.NET Core boş ilk segment yüzünden route'u eşleştirmez (canlıda 404; doğru path 400).
    /// `FlowTelemetry` tam bu tuzağa düşmüştü: telemetri hatayı yuttuğu için iOS kart ekleme
    /// olayları aylarca hiç kaydedilmedi, admin portaldaki huni yalnız Android'i saydı.
    static func url(origin: URL, path: String) -> URL {
        let trimmed = String(path.drop(while: { $0 == "/" }))
        return URL(string: origin.absoluteString + "/" + trimmed)
            ?? origin.appendingPathComponent(trimmed)
    }
}
