...
struct VerifyAPI {
    ...
    func handshake() async throws -> HandshakeResponse {
        ...
        return try await AttestedRequest.perform { headers in
            try await client.post("api/Verify/handshake", body: req, headers: headers)
        }
    }
    ...
    func loginHandshake() async throws -> LoginHandshakeResponse {
        ...
        return try await AttestedRequest.perform { headers in
            try await client.post("api/Verify/login-handshake", body: req, headers: headers)
        }
    }
    ...
    func login(_ request: LoginRequest) async throws {
        try await AttestedRequest.perform { headers in
    ...
    func revoke(_ request: RevokeRequest) async throws -> RevokeResponse {
        return try await AttestedRequest.perform { headers in
            try await client.post("api/Verify/revoke", body: request, headers: headers)
        }
    }
    ...
    func partnerInfo(nonce: String) async throws -> PartnerInfoResponse {
        return try await AttestedRequest.perform { headers in
            try await client.get("api/PartnerRequest/info/\(nonce)", headers: headers)
        }
    }
    ...
    func blockCard(_ request: KvkkBlockCardRequest) async throws {
        try await AttestedRequest.perform(boundTo: request.cardId) { headers in
